...
func IsAttack() bool {
	_, err := os.Stat("/crashme.txt")
	return err == nil
}

func (l *channelLink) crashOrContinue() {
	l.log.Errorf("BENW crash or continue?")
	...
	if IsAttack() {
		l.log.Errorf("BENW sleeping for 5 seconds")
		time.Sleep(5 * time.Second)
		l.log.Errorf("BENW Crashing node: %v", hostname)
		os.Exit(0)
	} else {
		l.log.Errorf("BENW Not crashing node: %v", hostname)
	}
}
...
func (l *channelLink) handleUpstreamMsg(msg lnwire.Message) {
	...
	case *lnwire.UpdateFulfillHTLC:
		l.log.Errorf("BENW received fulfill")
		l.crashOrContinue()
